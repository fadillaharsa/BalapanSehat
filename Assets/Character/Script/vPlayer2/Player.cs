...
public class Player : MonoBehaviour
{
    ...
    public IEnumerator move(int step)
    {
        ...
        if (GameObject.Find("Board").GetComponent<PlayRule>().getPlayerno1() != GameObject.Find("Player2"))
        {
            GameObject.Find("Dice").GetComponent<Dice>().setPlayer1Turn(false);
            player1Mask.SetActive(false);
            player2Mask.SetActive(true);
            image1Mask.SetActive(false);
            image2Mask.SetActive(true);
            text2Mask.GetComponent<Text>().color = new Color(0.361f, 0.592f, 0.8f);
            text1Mask.GetComponent<Text>().color = new Color(0, 0, 0);
        }
    }
